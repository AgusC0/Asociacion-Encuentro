...
<div id="parallax-noticias">
     <?php
     $cat=strval($_GET['cat']);
     $query=mysql_query("select * from category where categoryName='$cat' ");
     while($row=mysql_fetch_array($query))
     {?> 
     <h1 data-aos="fade-up" data-aos-duration="1000"><strong>Noticias > <?php echo html_entity_decode($row['categoryName']);?></strong></h1> 
     <?php   } ?>  
</div> 
...
<?php
$cat=strval($_GET['cat']);
$query=mysql_query("select * from noticias where categoriaNoticia='$cat' order by id DESC");
while($row=mysql_fetch_array($query))
{?>                   
                    <div class="card flex-row"> <a class="text-decoration-none" href="noticia.php?pid=<?php echo html_entity_decode($row['id']);?>"><img class="card-img-left img-fluid" src="../internoencuentro/vistas/modulos/privado/productimages/<?php echo html_entity_decode($row['id']);?>/<?php echo html_entity_decode($row['productImage1']);?>" /></a>
                        <div class="card-body" >
                            <h4 class="card-title h5 h4-sm"><i class="fa-solid fa-calendar-days"></i><span><?php echo html_entity_decode($row['fechaNoticia']);?></span><i class="fas fa-caret-right" aria-hidden="true"></i><span><?php echo html_entity_decode($row['categoriaNoticia']);?></span> </h4>
                            <a class="text-decoration-none" href="noticia.php?pid=<?php echo html_entity_decode($row['id']);?>"><h3 class="fw-bolder mb-2 mt-3 text-decoration-none" style="color:#000;"><?php echo html_entity_decode($row['tituloNoticia']);?></h3></a>
                          
                            <p class="card-text"><?php echo html_entity_decode($row['copeteNoticia']);?></p>
                        </div>
                    </div>
  <?php   } ?>  
<br> 
